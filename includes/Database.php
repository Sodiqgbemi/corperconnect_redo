...
class Database
{
    ...
    public function getRecFrmQry($query, $type = 'multi')
    {
        $stmt = $this->pdo->prepare($query);
        $stmt->execute();
        return $type == 'multi' ? $stmt->fetchAll(PDO::FETCH_ASSOC) : ($stmt->fetch(PDO::FETCH_ASSOC) ?: null);
    }
    ...
    public function getQueryCount($tableName, $field, $cond = ''): int
    {
    ...
    public function getRawQueryCount(string $sql): int {
        try {
    ...
    public function update($tableName, array $set, array $where, $operator = 'AND')
    {
        $arrSet = array_map(
            function ($key) {
                return $key . '=:' . $key;
        ...
        $arrWhere = array_map(
            function ($key) {
                return $key . '=:' . $key . '_where';
            },
            array_keys($where)
        );

        $whereClause = implode(" $operator ", $arrWhere);

        $stmt = $this->pdo->prepare(
            "UPDATE $tableName SET " . implode(', ', $arrSet) . " WHERE $whereClause"
        );
        ...
        foreach ($where as $field => $value) {
            $stmt->bindValue(':' . $field . '_where', $value);
        }
    ...
    public function dbval($query, array $params = [], $single = true)
    {
        try {
            $stmt = $this->pdo->prepare($query);
            $stmt->execute($params);
    ...
    /**
     * General method to run any SQL query
     * @param string $sql
     * @param array $params
     * @return bool
     */
